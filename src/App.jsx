...
function BrandMark({ compact = false }) {
  ...
  return <div className={`brand-mark ${compact ? 'compact' : ''}`} aria-label="Vemparajugari Vantillu">
    {!failed ? <img src={LOGO_PATH} alt="Vemparajugari Vantillu" onError={() => setFailed(true)} /> : <div className="logo-placeholder"><span>వేమ్పరాజుగారి</span><strong>వంటిల్లు</strong><small>VEMPRAJUGARI VANTILLU</small></div>}
  </div>;
}
...
function MenuPage({ active, setActive, language, setLanguage, search, setSearch, onHome, onItem }) {
  ...
  return <section className="menu-page"><header className="app-header"><button className="icon-button" onClick={onHome} type="button" aria-label="Back">←</button><BrandMark compact /><span /></header><div className="menu-heading"><div><p className="eyebrow">VEMPRAJUGARI VANTILLU</p><h1>{active === 'pickles' ? 'పచ్చళ్ళు' : 'రెస్టారెంట్ మెనూ'}</h1><span>{active === 'pickles' ? 'PICKLES' : 'RESTAURANT MENU'}</span></div><div className="language-switch" aria-label="Language">{['both','te','en'].map(v => <button key={v} className={language === v ? 'active' : ''} onClick={() => setLanguage(v)} type="button">{v === 'both' ? 'తెలుగు + English' : v === 'te' ? 'తెలుగు' : 'English'}</button>)}</div></div><div className="section-switcher">{categories.map(c => <button key={c.id} className={active === c.id ? 'active' : ''} onClick={() => { setActive(c.id); setSearch(''); }} type="button"><span>{c.icon}</span><strong>{c.label}</strong><small>{c.te}</small></button>)}</div><div className="search-box"><span>⌕</span><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search menu / మెనూలో వెతకండి" aria-label="Search menu" />{search && <button onClick={() => setSearch('')} type="button" aria-label="Clear search">×</button>}</div>{visibleSections.map(section => <div className="menu-section" key={section.id}><div className="section-title"><div><p>{active === 'pickles' ? 'PICKLES' : 'RESTAURANT'}</p><h2>{section.category}</h2><span>{section.te}</span></div><i /></div>{section.note && <p className="section-note">{section.note}</p>}<div className="menu-list">{section.items.map(item => <MenuItem key={item.id} item={item} language={language} onOpen={onItem} />)}</div></div>)}{!visibleSections.length && <div className="verified-empty"><strong>{active === 'pickles' ? 'Pickle details will be added after verification' : 'No matching items'}</strong><span>{active === 'pickles' ? 'Product names, pack sizes, prices and ingredients will be published after owner confirmation.' : 'Try another menu search.'}</span></div>}</section>;
}
